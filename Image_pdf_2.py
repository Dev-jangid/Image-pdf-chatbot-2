...
class Config:
    GROQ_API_KEY = st.secrets["GROQ_API_KEY"]   ### for streamlit hosting in secrets api and square bracket
...
def is_image_query(query):
    ...
    image_patterns = [
        r"(give me|show|display|view).*\b(image|picture|diagram|chart|figure|graph|photo)\b",
        r"(are there any|find|where is).*\b(images?|pic?|photo?|photos?|pictures?|diagrams?|charts?)\b",
